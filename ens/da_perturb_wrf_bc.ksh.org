...
#cys: need to set NL_ENSDIM_ALPHA=0, so that da_wrfvar.exe can run randomcv correctly
   ...
   if [[ ! -f $DA_FIRST_GUESS ]]; then
      cp /glade/scratch/junkyung/MPD_exp/pecan/wrf/$DATE/wrfinput_d01 $DA_FIRST_GUESS
   fi
...
cd $RUN_DIR
   ...
   if [[ ! -f ${RC_DIR_SAVE}/${DATE_SAVE}/wrfbdy_d01 ]]; then
      cp /glade/scratch/junkyung/MPD_exp/pecan/wrf/$DATE_SAVE/wrfbdy_d01  ${RC_DIR_SAVE}/${DATE_SAVE}/wrfbdy_d01 
   fi
...
export DATE=$DATE_SAVE
            ...
            date
            qsub $SCRIPTS_DIR/da_run_update_bc.ksh > $RUN_DIR/index.html 2>&1 &
            date
            RC=$?
            if [[ $? != 0 ]]; then
