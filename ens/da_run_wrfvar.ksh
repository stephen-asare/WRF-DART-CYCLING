...
# in ~/.kshrc  ─ or just before you source it
# if [[ -t 0 ]]; then
#     stty erase ^H         
# fi
...
#cmodule reset
# module use /opt/modulefiles/tcl
...
ml python/3
# module restore
echo "Debug Running da_run_wrfvar.ksh"
export REL_DIR=${REL_DIR:-$HOME/trunk}
export WRFVAR_DIR=${WRFVAR_DIR:-$REL_DIR/WRFDA}
export SCRIPTS_DIR=${SCRIPTS_DIR:-$WRFVAR_DIR/var/scripts}
echo "davar " ${SCRIPTS_DIR}
...
export DA_FIRST_GUESS=/gpfs/home/sa24m/scratch/tqprof/run2/osse_out/icbc/$DATE/wrfinput_d01_12_18 # wrfvar first guess.
...
#     export DA_FIRST_GUESS=${FC_DIR}/${PREV_DATE}/${FILE_TYPE}_d01_${ANALYSIS_DATE}
      if ! $RUN_WPB; then
         echo "Setting DA_FIRST_GUESS from previous cycle"
         export DA_FIRST_GUESS=${FC_DIR}/${PREV_DATE}/${FILE_TYPE}_d01_${ANALYSIS_DATE}
      fi
   fi
fi
if [[ $NL_MULTI_INC == 2 ]]; then
   echo "Setting DA_FIRST_GUESS and DA_BOUNDARIES for multi-inc"
...
#-------------------------------------------------------------------
mkdir trace
echo "DEBUG: Running WRF-VAR now"
if $DUMMY; then
   echo Dummy wrfvar
   ...
   if [[ $RC != 0 ]]; then
      echo "$(date)  da_wrfvar.exe failed, RC=$RC" >&2
      exit $RC
   fi
...
#     rm low_res_increment low_res_increment-down
   ...
   # rm -f oma_*.*
   # rm -f filtered_*.*
   echo "wrfvar_output is in $RUN_DIR/wrfvar_output"
   if [[ $NL_MULTI_INC == 0 ]] ; then
      if [[ -f wrfvar_output ]]; then
         if [[ $DA_ANALYSIS != wrfvar_output ]]; then 
            if ! $RUN_ADJ_SENS; then
               echo "Copying wrfvar_output to $DA_ANALYSIS"
               cp wrfvar_output $DA_ANALYSIS   #jkjk
...
echo "DEBUG: Finished WRF-VAR run"
echo '</PRE></BODY></HTML>'
