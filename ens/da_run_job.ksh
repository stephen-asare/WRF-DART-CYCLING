...
#   source /glade/u/home/junkyung/setup_env_dart.sh

export REL_DIR=${REL_DIR:-$HOME/trunk}
export WRFVAR_DIR=${WRFVAR_DIR:-$REL_DIR/WRFDA}
#export SCRIPTS_DIR=${SCRIPTS_DIR:-$WRFVAR_DIR/var/scripts}
export SCRIPTS_DIR=${SCRIPTS_DIR:-$WRFVAR_DIR/var/scripts/RC_ICBDY_ptb/wider_dm/july15_mod_as3_30h/d01}  #jk
echo "do_job" $SCRIPTS_DIR
. ${SCRIPTS_DIR}/da_set_defaults.ksh
export RUN_DIR=${RUN_DIR:-$EXP_DIR/$RUN}
export WORK_DIR=$RUN_DIR/working

export SCRIPT=${SCRIPT:-$SCRIPTS_DIR/da_run_wrfvar.ksh}
echo "DEBUG: Using script $SCRIPT"

mkdir -p $RUN_DIR
cd $RUN_DIR

if [[ $SUBMIT == "LoadLeveller" ]]; then 
   echo "DEBUG: Using LoadLeveller submission"
...
EOF
   ...
   echo "DEBUG: Using LSF submission"
   cat > job.ksh <<EOF
#!/bin/ksh
#
# LSF batch script
...
EOF
   ...
   echo "DEBUG: Using PBS submission"
   # Rather simplistic node calculation
...
EOF
   ...
   echo "DEBUG: Not submitting job, running directly"
   cat > job.ksh <<EOF
#!/bin/ksh
$SCRIPT > $RUN_DIR/index.html 2>&1
EOF
...
EOF
echo "Debug: Finalizing job script"
chmod +x job.ksh
...
if [[ $SUBMIT == "LoadLeveller" ]]; then 
   ...
   #sbatch $SUBMIT_FLAGS $PWD/job.ksh
else
echo "Running with $NUM_PROCS processors, output to $RUN_DIR"
   ./job.ksh
fi
