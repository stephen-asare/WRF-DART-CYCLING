...
export NETCDF=/opt/rcc/intel/openmpi
export HDF5=/opt/rcc/intel/hdf5-1.10.4
export PNETCDF=/gpfs/home/sa24m/stephen_asare/models/NETCDF
export NETCDF4=1
export WRFIO_NETCDF4_FILE_SUPPORT=1
export WRFIO_NCD_LARGE_FILE_SUPPORT=1
export WRF_EM_CORE=1
...
da_update_bc
```
**NB** If you do not see `da_update_bc`, yet `da_advance_time.exe` and `da_wrfvar.exe` is available, do not need to clean the build, just patch the config file the NetCDF libraries directly into `LIB_EXTERNAL` in the config file and Fire off the compilation again.
```
...
### Compiling Model Evaluation Tool (MET)
```bash
cd $SOFTWARE_DIR
mkdir MET/ && cd MET
```
...
export MET_NETCDF=/opt/rcc/intel/openmpi
export MET_HDF5=/opt/rcc/intel/hdf5-1.10.4
